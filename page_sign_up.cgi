...
sub sign_up_page
{
	...
	print	div({-id=>'centreDoc'},
				...
				fieldset({},
					...
					table({-cellpadding=>'3'},
						Tr(td({-width=>$table_width},"Username: ([\\w_]{6,})".td(textfield({-pattern=>'[\w_]{6,}'},"Username","",20,20)))."\n"),
						#Tr(td({-width=>$table_width},"Password: ([\\w_]{6,})".td(textfield({-pattern=>'[\w_]{6,}'},"Password","",20,20)))."\n"),
...
sub make_user
{
	...
	my @profile =('$f_name','$l_name','$gender','$year','$month','$day','$email','$hair','$degree','$height','$weight','$u_name','$pass');
	foreach (@profile) {
		$_ =~ s/'/''/g;
		$_ =~ s/"/""/g;
		$_ =~ s/[^\_\"\'\w\d\s]//;
	...
	$to = 'brady.a.watkinson@gmail.com';
	$from = 'webmaster@love2041.com';
